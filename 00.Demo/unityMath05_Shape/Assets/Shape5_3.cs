...
public class Shape5_3 : MonoBehaviour {
    ...
    const float fWaveLength = 5.0f;         // 波長
    const float fWaveAmp = 1.0f;            // 振幅
    const float fPeriod = 2000.0f;          // 周期
    private Vector3 k = new Vector3( 1.0f, 0.0f, 0.0f );  // 波数ベクトル
    private float fWaveAngle = 0.0f;        // 波の進行方向
    private float fWaveAngleVel = 2.0f * 3.14159f / 100.0f;
    // 頂点
    ...
    void FixedUpdate()
    {
        fWaveAngle += Input.GetAxis("Horizontal") * fWaveAngleVel;
        k = new Vector3(Mathf.Cos(fWaveAngle), 0.0f, Mathf.Sin(fWaveAngle)) / fWaveLength;
        ...
        {
            float fPhase = fMiliSec / fPeriod - Vector3.Dot(k, positions[i]);
